...
class FBoolector(Boolector):
    ...
    def fMul(self, nodeA, nodeB):
        ...
        log = super().Var(super().BitVecSort(self.fptype.value[MAN] + 1))
        super().Assert(super().Eq(super().Const(1, self.fptype.value[MAN] + 1), super().Srl(man, log)))
        
        super().Assert(super().Eq(self.fExponent(var),
            super().Sub(
                super().Add(self.fExponent(nodeA), self.fExponent(nodeB)),
                super().Add(
                    super().Const(2**(self.fptype.value[EXP]-1)-1, self.fptype.value[EXP]),
                    super().Sub(log, super().Const(self.fptype.value[MAN], self.fptype.value[EXP]))))))
        #TODO over- / underflow
        
        prec = super().Var(BitVecSort(self.fptype.value[MAN]))
        super().Assert(super().Eq(
            super().Concat(self.fMantisseIm(var), prec),
            super().Mul(
                super().Concat(super().Const(0, self.fptype.value[MAN]), self.fMantisseIm(nodeA)),
                super().Concat(super().Const(0, self.fptype.value[MAN]), self.fMantisseIm(nodeB)))))
        #TODO rounding
        return var
